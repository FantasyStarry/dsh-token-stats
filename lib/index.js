...
export default {
  ...
  apply(ctx, config = {}) {
    ...
    let keepDays = Number.isInteger(config.keepDays) && config.keepDays > 0 ? config.keepDays : 366;
    ...
    if (installSettingsSection && STATS_SETTINGS_NAMESPACE && Config) {
      installSettingsSection(ctx, STATS_SETTINGS_NAMESPACE, Config, config, {
        ...
        onChange: () => {
          ...
          const nextKeep = Number.isInteger(next.keepDays) && next.keepDays > 0 ? next.keepDays : 366;

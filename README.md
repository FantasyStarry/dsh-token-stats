...
- **四处展示（v0.2.0 起，数字用 k/m/b 缩写）**：
  1. **悬浮用量宠物（v0.6.0，`shell.overlay`）**：右下角一只圆滚滚的史莱姆
     ...
     - **工作中（v0.7.0）**：服务端把 `activity`（上次活动时刻 + 最近完成列表）
       ...
       宠物就"专注盯小电脑"（表情 + 打字颠簸 + 右上角脉冲点），悬停气泡显示
       「正在干活…」；
     - **空闲**：`0 请求 → 打瞌睡`、`轻用量 → 休息眯眼`、`<300k → 冒汗`、
       `≥300k → 晕眩`（阈值可改 `src/client.ts` 的 `MOOD_BUSY_AT` /
       `MOOD_DIZZY_AT`）；
     - **完成提示（v0.7.0）**：多任务/子代理每完成一次模型调用，弹
       `✅ 任务完成 · 输入/输出 · 子代理` 提示 + 扩散光环 + 蹦一下
       （普通单会话回复不打扰）；整段多任务全部收工弹 `💤 收工啦～`；
     - 悬停吐泡泡显示一行摘要，点击弹出今日汇总面板（计费输入/输出/请求 +
       缓存命中率 + 7 天迷你柱状图 + 设置页指引），再点或点外部/Escape 收起。
       可拖拽移动，位置与可见性持久化 localStorage（默认右下角）；
  2. 侧边栏底部"用量宠物"开关（v0.6.0 起替代常驻数字小部件）：显示/隐藏悬浮宠物，
     隐藏后宠物完全消失，界面不占用任何空间；
  3. 设置页"用量统计"分区（v0.3.1 起为克制数据面板：无卡片盒子，复用宿主 CSS
     变量，深浅色自适应）：

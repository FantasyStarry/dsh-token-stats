...
/** 单次模型调用的用量（assistant/message 事件）。 */
interface Usage {
  requests?: number;
  inputTokens?: number;
  outputTokens?: number;
...
interface StatsState {
  days: Record<string, Record<string, Record<string, Stats>>>;
  sessions: Record<string, Record<string, SessionStats>>;
}
...
interface LogEvent {
  ...
  time?: number;
  data?: { usage?: Usage; message?: { source?: { provider?: string; model?: string } } };
}
...
interface TodayEvent {
  ...
  usage: Usage;
  time: number;
  meta: { id: string; parent?: string; subagent: boolean } | null;
}

/** 实时订阅的 session 参数（仅用到 id/header）。 */
...
/** 把一份 usage 累加进目标桶。 */
function addStats(target: Stats, usage: Stats): void {
...
function ensureModel(state: StatsState, day: string, provider: string, model: string): Stats {
...
function loadState(path: string): StatsState {
  const state: StatsState = { days: {}, sessions: {} };
...
/** 原子持久化（tmp + rename）。 */
function persistState(path: string, state: StatsState): void {
...
function historyFor(state: StatsState, days: number): { days: { day: string; total: Stats }[] } {
  const result = [];
  const now = Date.now();
...
/** 裁剪过旧数据，防止文件无限增长。 */
function prune(state: StatsState, keepDays: number): void {
  ...
  if (state.sessions) {
    for (const day of Object.keys(state.sessions)) {
      if (day < cutoff) delete state.sessions[day];
    }
  }
}
 ...
 * 用于标记事件所属会话及其子代理身份。
 */
function collectTodayUsage(start: number): TodayEvent[] {
  ...
  apply(ctx: CordisLike, config: PluginConfig = {}) {
    ...
    const record = (
      ...
      if (meta && meta.id) {
        ...
        if (meta.parent) s.parent = meta.parent;
        if (typeof meta.subagent === "boolean") s.subagent = meta.subagent;
        s.requests += 1;
    ...
    if (installSettingsSection && STATS_SETTINGS_NAMESPACE && Config) {
      installSettingsSection(ctx as any, STATS_SETTINGS_NAMESPACE, Config, config, {
        setSource: (source) => {
          current = source;
        },
        onChange: () => {
          ...
          const nextKeep =
            typeof next.keepDays === "number" && Number.isInteger(next.keepDays) && next.keepDays > 0
              ? next.keepDays
              : 366;

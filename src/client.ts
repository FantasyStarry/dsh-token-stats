...
/** 统计桶形状（summary/history 共用；cost = 估算费用（元），null = 未计价）。 */
interface StatsLike {
	requests: number;
	inputTokens: number;
...
/** /token-stats/balance 响应（官方 GET /user/balance，服务端 60s 缓存）。 */
interface BalanceData {
	ok: boolean;
	isAvailable?: boolean;
	currency?: string;
	total?: number;
	infos?: { currency?: string; total?: string; granted?: string; toppedUp?: string }[];
	fetchedAt?: number;
	error?: string;
	message?: string;
}
...
window.__ModuleLoader__.load({
	...
	factory: (require: (id: string) => any) => {
		...
		const keyOfDate = (d: Date): string =>
			`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;

		/** 所在周的周一（00:00）。 */
		const mondayOnOrBefore = (d: Date): Date => {
		 ...
		 */
		const buildHeatGrid = (data: HistoryData | null): HeatGrid => {
			...
			const levelOf = (v: number): number => (v <= 0 ? 0 : v <= th[0] ? 1 : v <= th[1] ? 2 : v <= th[2] ? 3 : 4);
			const cols: HeatCol[] = [];
		...
		let miniHeatData: HistoryData | null = null;
		let miniHeatLoading = false;
		const miniHeatListeners = new Set<() => void>();
		const miniHeatLoad = (): void => {
			if (miniHeatData || miniHeatLoading) return;
			miniHeatLoading = true;
			fetchJson("/token-stats/history?days=210")
				.then((d: HistoryData) => {
					miniHeatData = d;
					miniHeatLoading = false;
					for (const cb of miniHeatListeners) cb();
				})
				.catch(() => {
					miniHeatLoading = false;
					for (const cb of miniHeatListeners) cb();
		...
		const miniHeatSubscribe = (cb: () => void): (() => void) => {
			miniHeatListeners.add(cb);
			return () => {
				miniHeatListeners.delete(cb);
			};
		};
		const miniHeatSnapshot = (): number => (miniHeatData ? 1 : 0);
...
`;
		...
		function SlimeSvg(props: { size: number; mood: Mood; bounceKey?: number; perk?: boolean; pokeKey?: number }) {
			...
			const face = (() => {
				...
				if (mood === "dizzy") {
					...
					const spiral = (key: string, cx: number) =>
			...
			return react.createElement(
				"svg",
				{
					...
					"aria-hidden": true,
					className:
						[bounceKey ? "ts-pet-bounce" : mood === "working" ? "ts-pet-work" : mood === "sleepy" ? "ts-pet-sleep" : undefined, "ts-pet-svg", perk ? "ts-pet-perk" : undefined]
							.filter(Boolean)
							.join(" ") || undefined,
				...
				[
					react.createElement(
						...
						react.createElement(
							"radialGradient",
							{ key: "grad", id: gid, cx: "32%", cy: "24%", r: "88%" },
							[
					...
					react.createElement(
						"g",
						{ key: "sprout", className: perk ? "ts-pet-sprout-fast" : "ts-pet-sprout" },
						[
					...
					react.createElement(
						"g",
						{ key: "float", className: "pt-float" },
						react.createElement(
							"g",
							{ key: "wobble", className: mood === "dizzy" ? "pt-dizzy-wobble" : undefined },
							react.createElement(
								"g",
					...
					pokeKey
						? [["♥", 30, 18, 9, 0], ["♥", 32.5, 13, 9, 0.22], ["♥", 35, 8, 9, 0.44], ["♥", 33, 4, 11, 0.7]].map(([h, x, y, sz, d], i) =>
								react.createElement(
									"text",
									{
										key: `h${i}`,
		...
		function TokenStatsPet(props: Record<string, unknown>) {
			...
			const [poke, setPoke] = react.useState<{ key: number } | null>(null);
			const pokeTimer = react.useRef<ReturnType<typeof setTimeout> | null>(null);
			const lastTap = react.useRef<{ t: number } | null>(null);
								...
								[
									react.createElement(
										...
										[
											react.createElement(
												"span",
												{ key: "t", style: { display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 700, color: STYLE.labelPrimary } },
												[
											...
											(
												() => {
													...
													const entry = (
														...
														} as Record<Mood, { dot: string; text: string }>
													)[lm];
													return react.createElement(
														"span",
														{ key: "mood", style: { display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, color: STYLE.labelTertiary, flex: "none" } },
														[
											...
											? react.createElement(
													...
													[
														...
														react.createElement(
															...
															`${total.reasoningTokens > 0 ? `推理 ${fmtCompact(total.reasoningTokens)} · ` : ""}${typeof total.cost === "number" ? `估算费用 ¥${fmtMoney(total.cost)} · ` : ""}平均输入 ${fmtCompact(billedTotal / Math.max(1, total.requests))}/请求`
														),
														react.createElement(
															"div",
															{ key: "hit", style: { marginTop: 10 } },
															[
																react.createElement(
																	"div",
																	{ key: "l", style: { display: "flex", justifyContent: "space-between", fontSize: 10.5, color: STYLE.labelTertiary, marginBottom: 4 } },
																	["缓存命中率", `${hitRate.toFixed(0)}%`]
																),
																react.createElement(
																	"div",
																	{ key: "t", style: { height: 4, borderRadius: 2, background: STYLE.surfaceL2, overflow: "hidden" } },
																	react.createElement("div", { key: "f", style: { width: `${Math.min(100, hitRate)}%`, height: "100%", borderRadius: 2, background: STYLE.accent, opacity: 0.8 } })
																)
															]
														),
														miniGrid
															? react.createElement(
																"div",
																{ key: "mheat", style: { marginTop: 12 } },
																[
																	react.createElement(
																		"div",
																		{ key: "t", style: { display: "flex", justifyContent: "space-between", fontSize: 10.5, color: STYLE.labelTertiary, marginBottom: 5 } },
																		[
																	...
																	react.createElement(
																		"div",
																		{ key: "g", className: "dts-mini-heat", style: { display: "flex", gap: 2, overflowX: "auto", paddingBottom: 2 } },
																		miniGrid.cols.map((c, i) =>
																			react.createElement(
																				"div",
																				{ key: i, style: { display: "flex", flexDirection: "column", gap: 2, flex: "none" } },
																				c.cells.map((cell) =>
																					react.createElement("div", {
																						key: cell.key,
														...
														react.createElement(
															...
															`近 7 天合计 ${fmtCompact(chartDays.reduce((a, d) => a + d.input, 0))} · 完整明细见 设置 → 用量统计`
														)
													]
												)
											: react.createElement(
													"div",
													{ key: "empty", style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 6, color: STYLE.labelSecondary, fontSize: 12.5, marginTop: 10 } },
													react.createElement(SlimeSvg, { size: 40, mood: "sleepy" }),
													"今天还没有记录到模型调用。"
												)
								]
							)
		 ...
		 */
		const stUseHistory = (days: number): { month: HistoryData | null; load: () => void; loading: boolean } => {
			const [month, setMonth] = react.useState<HistoryData | null>(null);
		...
		function TokenStatsSectionPolished(props: Record<string, unknown>) {
			...
			react.useEffect(() => {
				if (stHeatOpen) heat.load();
			}, []);
			/** 热力图默认滚动到最右（今天），窄容器下无需手动找。 */
			const heatWrapRef = react.useRef<HTMLDivElement | null>(null);
			react.useEffect(() => {
			...
			const [heatTip, setHeatTip] = react.useState<{ cell: HeatCell; x: number; y: number } | null>(null);
			/** 会话 ID 点击复制（1.5s ✓ 反馈）。 */
			...
			const [past, setPast] = react.useState<{ summary: SummaryData | null; sessions: SessionsData | null; error: string | null } | null>(null);
			...
			react.useEffect(() => {
				...
				Promise.all([
					...
					if (!alive) return;
					setPast({
						summary: pair[0] as SummaryData | null,
						sessions: pair[1] as SessionsData | null,
			...
			const shiftDay = (delta: number): void => {
				...
				if (next > todayKey() || next < minDay) return;
			...
			const tdNum = { ...tdStyle, ...NUM, textAlign: "right" as const };

			const sessList: SessionRow[] = activeSessions && Array.isArray(activeSessions.sessions) ? activeSessions.sessions : [];
			...
			const deltaEl = (() => {
				if (yestIn < 0) return null;
				if (yestIn === 0 && todayIn > 0) return react.createElement("span", { className: "dts-delta" }, "较昨日 新增");
				if (yestIn === 0) return react.createElement("span", { className: "dts-delta" }, "较昨日 持平");
			...
			react.useEffect(() => {
				if (lastUpdated > 0) setRefreshing(false);
			}, [lastUpdated]);
			react.useEffect(
				() => () => {
			...
			const onRefresh = () => {
				setRefreshing(true);
				reload();
				if (!isToday) setPastTick((t2) => t2 + 1);
				if (refreshTimer.current) clearTimeout(refreshTimer.current);
			...
			const heading = (title: string, note?: string) =>
				react.createElement(
					"div",
					{ className: "dts-section-heading" },
				[title, note ? react.createElement("small", { key: "note" }, note) : null]
			);

			/** 次级指标格：小标签 + 数值（复用 .dts-metric 分隔线样式，替代 statGroup）。 */
			const dtsMetric = (key: string, label: string, value: string) =>
				react.createElement(
					"div",
					{ key, className: "dts-metric" },
					[
			...
			const stMonthSection = (
				...
				const head = react.createElement(
					"div",
					{ className: "dts-section-heading dts-section-heading-toggle", onClick: toggle, role: "button", tabIndex: 0, onKeyDown: (e: { key: string; preventDefault: () => void }) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggle(); } }, "aria-expanded": open },
					[
				...
				const body = !data && loading
					...
					: data && data.days && data.days.length > 0
						? react.createElement(
								"div",
								{ key: "m-scroll", className: "dts-table-scroll" },
								react.createElement(
									"table",
									{ style: { ...tableStyle, minWidth: 460 } },
									[
										react.createElement(
											"thead",
											{ key: "head" },
			...
			const stHeatSection = () => {
				...
				const heatByKey = new Map(grid.cols.flatMap((c) => c.cells.map((cell) => [cell.key, cell] as const)));
				const pickCell = (cell: HeatCell): void => {
				...
				const toggle = () => {
					...
					stSetHeatOpen(next);
					if (next && !heat.month) heat.load();
				};
				const head = react.createElement(
					"div",
					{ className: "dts-section-heading dts-section-heading-toggle", onClick: toggle, role: "button", tabIndex: 0, onKeyDown: (e: { key: string; preventDefault: () => void }) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggle(); } }, "aria-expanded": stHeatOpen },
					[
				...
				const body = !heat.month
					...
					: react.createElement(
						"div",
						{ key: "h-box", className: "dts-heat-box" },
						[
							react.createElement(
								"div",
								{ key: "scroll", ref: heatWrapRef, className: "dts-heat-wrap" },
								[
									react.createElement(
										"div",
										{ key: "mrow", className: "dts-heat-row" },
										[
											react.createElement("div", { key: "sp", className: "dts-heat-wd", "aria-hidden": true }),
											react.createElement(
												"div",
												{ key: "months", className: "dts-heat-months" },
												grid.cols.map((c, i) =>
									...
									react.createElement(
										"div",
										{ key: "grow", className: "dts-heat-row" },
										[
											react.createElement(
												"div",
												{ key: "wd", className: "dts-heat-wd", "aria-hidden": true },
												["一", "", "三", "", "五", "", ""].map((w, i) =>
													react.createElement("span", { key: i, className: "dts-heat-wd-item" }, w)
												)
											),
											react.createElement(
												"div",
												{
													key: "grid",
													className: "dts-heat",
													onMouseOver: (e: { target: EventTarget | null }) => {
														const el = (e.target as HTMLElement | null)?.closest(".dts-heat-cell") as HTMLElement | null;
														if (!el) return;
				...
				const tipEl = heatTip
					? react.createElement(
						...
						[
							...
							react.createElement(
								"div",
								{ key: "v", style: { display: "flex", alignItems: "center", gap: 6, marginTop: 4 } },
								[
			...
			const sessionTable = (list: SessionRow[]) =>
				list.length === 0
					? null
					: react.createElement(
						"div",
						{ className: "dts-table-scroll dts-tbl-session" },
						react.createElement(
							"table",
							{ style: tableStyle },
							[
									...
									list.map((s) => {
										...
										return react.createElement(
											...
											[
												react.createElement(
													"td",
													{ key: "id", className: "dts-cell-ellipsis dts-cell-copy", style: { ...tdStyle, ...NUM, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis" }, title: `${s.id}（点击复制完整 ID）`, onClick: () => copySessionId(s.id) },
													[
														...
														s.subagent
															? react.createElement(
																"span",
																{ key: "parent", style: { display: "inline-flex", alignItems: "center", gap: 4, marginLeft: 6, whiteSpace: "nowrap" } },
																[
												...
												react.createElement(
													"td",
													{ key: "share", style: tdNum },
													react.createElement(
														"div",
														{ style: { display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 6 } },
														[shareBar(pct(billed, billedTotal)), react.createElement("span", { key: "pct", style: { color: STYLE.labelSecondary } }, `${pct(billed, billedTotal).toFixed(0)}%`)]
													)
												)
											]
										);
			...
			const modelTable = react.createElement(
				"div",
				{ className: "dts-table-scroll dts-tbl-model" },
				react.createElement(
					"table",
					{ style: tableStyle },
					[
						react.createElement(
							...
							react.createElement(
								"tr",
								null,
								["提供商", "模型", "计费输入", "未缓存", "缓存读", "输出", "推理", "请求", "费用", "占比"].map((h, i) =>
								...
								Object.entries(pv.models).map(([model, ms]) => {
									...
									return react.createElement(
										...
										[
											...
											react.createElement(
												"td",
												{ key: "c", style: { ...tdNum, color: typeof ms.cost === "number" ? STYLE.labelPrimary : STYLE.labelTertiary }, title: typeof ms.cost === "number" ? `估算费用 ¥${ms.cost.toFixed(4)}（按参考价）` : "该模型未配置价格（插件配置 → prices 可设置）" },
												typeof ms.cost === "number" ? `¥${fmtMoney(ms.cost)}` : "—"
											),
											react.createElement(
											"td",
											{ key: "s", style: tdNum },
											react.createElement(
												"div",
												{ style: { display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 6 } },
												[shareBar(pct(billed, billedTotal)), react.createElement("span", { key: "t", style: { color: STYLE.labelSecondary } }, `${pct(billed, billedTotal).toFixed(0)}%`)]
											)
										)
										]
									);
			...
			const historyTable = history && history.days && history.days.length > 0
				? react.createElement(
						"div",
						{ className: "dts-table-scroll", style: { marginTop: 10 } },
						react.createElement(
							"table",
							{ style: { ...tableStyle, minWidth: 560 } },
							[
								react.createElement(
									"thead",
									{ key: "head" },
									react.createElement(
										"tr",
										null,
										["日期", "计费输入", "费用", "缓存读", "输出", "请求"].map((h, i) => react.createElement("th", { key: h, style: i === 0 ? thLeft : thStyle }, h))
									)
								),
								react.createElement(
									"tbody",
			...
			/** 空态（按所选天文案）。 */
			const emptyStateEl = react.createElement(
				"div",
				{ key: "empty", className: "dts-empty" },
				react.createElement(SlimeSvg, { size: 46, mood: "sleepy" }),
				isToday ? "今天还没有记录到模型调用。" : `${effDay} 没有记录到模型调用。`
			);

			return react.createElement(
				"section",
				{ className: "dts-settings", "aria-labelledby": "dts-settings-title" },
				[
					react.createElement("style", { key: "style" }, css),
					react.createElement(
						"div",
						{ key: "head", className: "dts-overview-head", style: { marginBottom: 14 } },
						[
							react.createElement(
								"div",
								{ key: "copy" },
							...
							react.createElement(
								"div",
								{ key: "actions", style: { display: "flex", alignItems: "center", gap: 8, flex: "none" } },
								[
									react.createElement(
										"div",
										{ key: "daynav", className: "dts-daynav" },
										[
									...
									react.createElement(
										"button",
										{ key: "csv", type: "button", onClick: exportCsv, className: "dts-refresh", title: "导出明细 CSV（模型+会话）", "aria-label": "导出明细 CSV" },
										"⤓"
									),
									react.createElement(
										"button",
										{ key: "refresh", type: "button", onClick: onRefresh, className: `dts-refresh${refreshing ? " spin" : ""}`, title: "刷新统计", "aria-label": "刷新统计" },
										"↻"
									),
									isToday && lastUpdated
					...
					activeError
						...
						: !activeSummary
							? react.createElement(
									"div",
									{ key: "loading", className: "dts-loading", "aria-live": "polite" },
									[
							...
							: [
								total && total.requests > 0 ? react.createElement(react.Fragment, { key: "day" }, [
									react.createElement(
										"div",
										{ key: "overview", className: "dts-overview" },
										[
											react.createElement(
											"div",
											{ key: "metrics", className: "dts-metric-grid" },
											[
									...
									react.createElement(
										"div",
										{ key: "subline", className: "dts-subline" },
										[
											estCost !== null
												? react.createElement(
														"span",
														{ key: "cost", title: "按模型参考价估算（内置 DeepSeek 空闲时段价，高峰 ×2）；可在 插件配置 → prices 按模型覆盖" },
														[
															"估算费用 ",
															react.createElement("strong", { key: "v", style: { color: STYLE.accent } }, `¥${fmtMoney(estCost)}`),
															...((unpriced > 0 && total ? [`（${unpriced} 个模型未计价）`] : []) as string[])
														]
													)
												: null,
											balTotal !== null
												? react.createElement(
														"span",
														{ key: "bal", title: `DeepSeek 官方 GET /user/balance（服务端 60s 缓存）${balance && balance.isAvailable === false ? "\n账户不可用：余额不足或未充值" : ""}` },
														[
															"官方余额 ",
									...
									chartDays.length > 1
										? react.createElement(
											...
											[
												...
												react.createElement(
													"svg",
													{ key: "s", className: "dts-spark", viewBox: "0 0 100 36", preserveAspectRatio: "none", "aria-hidden": true },
													[
														react.createElement(
															"defs",
															{ key: "defs" },
															react.createElement(
																"linearGradient",
																{ key: "g", id: sparkGid, x1: 0, y1: 0, x2: 0, y2: 1 },
																[
								...
								providerRows.length > 0
									? react.createElement(
											"div",
											{ key: "prov-chips", className: "dts-provider-chips", "aria-label": "提供商汇总" },
											providerRows.map((p) => {
												const ratio = billedTotal > 0 ? Math.max(0, Math.min(100, (p.billed / billedTotal) * 100)) : 0;
												return react.createElement(
													"span",
													{ key: p.name, className: "dts-provider-chip", title: `${p.name}\n计费输入 ${fmt(p.billed)}\n请求 ${fmt(p.requests)}` },
													[
								...
								react.createElement(
									"div",
									{ key: "chart-wrap", className: "dts-chart-wrap" },
									[
